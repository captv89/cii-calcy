...
await page.getByLabel('Gross Tonnage:').fill('325678');
await page.getByLabel('Gross Tonnage:').click();
await page.getByLabel('Gross Tonnage:').fill('305678');
await page.getByLabel('Rating Year:').click();
await page.getByLabel('Rating Year:').fill('2023');
await page.getByLabel('Distance:').click();
await page.getByLabel('Distance:').fill('25000');
await page.getByRole('combobox', { name: 'Fuel:' }).selectOption('LFO');
...
await page
	.locator('form div')
	.filter({
		hasText: 'Fuel Consumption 2 Distance: Fuel: Select Fuel TypeDiesel/Gas OilLFOHFOLPG(Propa'
	})
	.getByLabel('Distance:')
	.click();
await page
